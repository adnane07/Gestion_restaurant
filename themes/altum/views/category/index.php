...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h1 class="h4 text-truncate mb-0"><?= sprintf(l('category.header'), $data->category->name) ?></h1>
        ...
        <div class="ml-3">
            <?php if($this->user->plan_settings->items_limit != -1 && $data->total_items >= $this->user->plan_settings->items_limit): ?>
                <button type="button" data-toggle="tooltip" title="<?= l('global.info_message.plan_feature_limit') ?>" class="btn btn-sm btn-primary disabled">
                    <i class="fa fa-fw fa-sm fa-plus"></i> <?= l('item.create') ?>
                </button>
            <?php else: ?>
                <a href="<?= url('item-create/' . $data->category->category_id) ?>" class="btn btn-sm btn-primary"><i class="fa fa-fw fa-sm fa-plus"></i> <?= l('item.create') ?></a>
            <?php endif ?>
        </div>
    </div>
                    ...
                    <div class="card h-100">
                        <div class="card-body d-flex flex-column justify-content-between">
                            ...
                            <p class="m-0">
                                <small class="text-muted">
                                    <i class="fa fa-fw fa-sm fa-dollar-sign text-muted mr-1"></i> <?= sprintf(l('item.price_currency'), $row->price, $data->store->currency) ?>
                                </small>
                            </p>
                            ...
                            <?php if($this->user->plan_settings->ordering_is_enabled): ?>
                                <p class="m-0">
                                    <small class="text-muted">
                                        <i class="fa fa-fw fa-sm fa-bell text-muted mr-1"></i> <a href="<?= url('orders/' . $row->store_id) ?>" target="_blank"><?= sprintf(l('item.orders'), nr($row->orders)) ?></a>
                                    </small>
                                </p>
                            <?php endif ?>
                        ...
                        <div class="card-footer bg-gray-50 border-0">
                            <div class="d-flex flex-lg-row justify-content-lg-between">
                                <div>
                                    <i class="fa fa-fw fa-sm fa-chart-pie text-muted mr-1"></i> <a href="<?= url('statistics?item_id=' . $row->item_id) ?>"><?= sprintf(l('category.pageviews'), nr($row->pageviews)) ?></a>
                                </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-column align-items-center justify-content-center py-3">
                    <img src="<?= ASSETS_FULL_URL . 'images/no_rows.svg' ?>" class="col-10 col-md-7 col-lg-4 mb-3" alt="<?= l('category.no_data') ?>" />
                    <h2 class="h4 text-muted"><?= l('category.no_data') ?></h2>
                    <p class="text-muted"><?= l('category.no_data_help') ?></p>
                </div>
            </div>
        </div>
    <?php endif ?>
